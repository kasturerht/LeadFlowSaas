const fs = require('fs');
const path = 'app/src/main/java/com/nexaleads/app/data/repository/LeadRepository.kt';
let content = fs.readFileSync(path, 'utf8');

// 1. Add ordersCol
content = content.replace('fun leadsCol() = db.collection("organizations").document(orgId).collection("leads")', 'fun leadsCol() = db.collection("organizations").document(orgId).collection("leads")\n    fun ordersCol() = db.collection("organizations").document(orgId).collection("orders_data")');
...
;
content = content.replace('suspend fun updateLeadAndAddInteractionBatch', newMethods + '\n    suspend fun updateLeadAndAddInteractionBatch');

// 3. Replace Dash Flow
const newDash =     fun getDashboardMetricsFlow(userId: String): Flow<Map<String, Long>> = callbackFlow {
...
content = content.replace(/fun getDashboardMetricsFlow.*?awaitClose \{ listener\.remove\(\) \}\n    \}/s, newDash);

// 4. Replace Sales Flow
const newSales =     fun getSalesMetricsFlow(userId: String): Flow<Map<String, Long>> = callbackFlow {
            ...
            .addSnapshotListener { snapshot, _ ->
                if (snapshot != null) {
                    ...
                    for (doc in snapshot.documents) {
                        ...
                        val cAt = (order.createdAt && order.createdAt.length >= 10) ? order.createdAt.substring(0, 10) : "";
                        if (cAt === "") continue;
                        if (cAt >= todayStr) {
                            todayCount++
...
content = content.replace(/fun getSalesMetricsFlow.*?awaitClose \{ listener\.remove\(\) \}\n    \}/s, newSales);

fs.writeFileSync(path, content);
console.log('Successfully updated LeadRepository.kt');
